(function () {
  ...
  function buildIndex(list) {
    return list.map(function (o) {
      var hay = [o.no, o.title, o.lv1, o.lv2, o.hint, o.mnemonic, o.cho, o.mean]
        .filter(Boolean).join(" ");
  ...
  function rowsHtml(list, ix, q) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      out += '<button type="button" data-i="' + i + '">' +
        ...
        (list[i].stars ? stars(list[i].stars)
          : list[i].kind ? '<span class="kind">' + esc(list[i].kind) + "</span>" : "") +
        "</button>";
    }
  ...
  var views = { quiz: $("view-quiz"), con: $("view-con"), read: $("view-read") };
  var tabs = { quiz: $("tab-quiz"), con: $("tab-con"), read: $("tab-read") };
  ...
  function render() {
    ...
    // 연상문장은 단계가 아니라, 1수준부터 카드 아래에 계속 떠 있는 힌트
    html += '<div class="card-foot">' +
      (open >= 1 && c.hint
        ? '<div class="hint' + (open === 1 ? " new" : "") + '">' + esc(c.hint) + "</div>"
        : "") +
      '<div class="foot-row">' +
        '<span class="tap-hint">' +
        ...
        "</span>" +
        '<span class="refs">' +
          '<button type="button" class="ref" data-doc="' + esc(c.no) + '">단문 ' +
          esc(c.no) + " →</button></span>" +
      "</div>" +
    "</div>";
  ...
  $("card").addEventListener("click", function (e) {
    ...
    if (r) { e.stopPropagation(); return toDoc(r.dataset.doc); }
    step();
  });
  ...
  function drawSheet() {
    var L = sheetMode === "con" ? CONS : CARDS;
    var X = sheetMode === "con" ? CON_IX : CARD_IX;
    $("sheet-list").innerHTML = rowsHtml(L, X, $("q").value);
  }

  function openSheet(mode) {
    sheetMode = mode === "con" ? "con" : "card";
    $("q").value = "";
    drawSheet();
    $("sheet").hidden = false;
  ...
  $("go").addEventListener("click", function () { openSheet("card"); });
  $("c-go").addEventListener("click", function () { openSheet("con"); });
  ...
  function cBuild() {
    var k = $("c-kind").value;
    cDeck = CONS.filter(function (o) { return !k || o.kind === k; });

    if ($("c-order").value === "random") {
      for (var i = cDeck.length - 1; i > 0; i--) {
        var j = Math.floor(Math.random() * (i + 1));
        var t = cDeck[i]; cDeck[i] = cDeck[j]; cDeck[j] = t;
      }
    }
    cAt = 0;
    cOpen = 0;
  ...
  function cRender() {
    ...
    if (!cDeck.length) {
      host.innerHTML = '<div class="empty">해당하는 개념이 없습니다.</div>';
      $("c-count").textContent = "";
      $("c-prev").disabled = $("c-next").disabled = true;
      return;
    }
    ...
    var html =
      '<div class="card-head">' +
        '<span class="card-no">' + esc(o.no) + "</span>" +
    ...
    var bodies = [
      '<div class="lv1 plain">' + esc(o.cho) + "</div>",
      '<div class="mean">' + esc(o.mean) + "</div>",
    ];

    for (var i = 0; i < cOpen; i++) {
      html += '<div class="layer' + (i === cOpen - 1 ? " new" : "") + '">' +
        '<span class="layer-tag">' + C_LAYERS[i] + "</span>" +
        bodies[i] + "</div>";
    }
    ...
    // 출처 단문으로 바로 건너뛸 수 있게
    var refs = (o.src || []).map(function (n) {
      return '<button type="button" class="ref" data-doc="' + esc(n) + '">단문 ' + esc(n) + " →</button>";
    }).join("");

    html += '<div class="card-foot"><div class="foot-row">' +
      '<span class="tap-hint">' +
        (cOpen < C_LAYERS.length ? "탭하면 " + C_LAYERS[cOpen] : "탭하면 다음 개념") +
      "</span>" +
      (refs ? '<span class="refs">' + refs + "</span>" : "") +
      "</div></div>";

    host.innerHTML = html;
    $("c-count").textContent = cAt + 1 + " / " + cDeck.length;
    $("c-prev").disabled = cAt === 0;
    $("c-next").disabled = cAt === cDeck.length - 1;
  }
  ...
  // 단문 원문에서 같은 번호의 두문자 카드로
  function toCard(no) {
    for (var i = 0; i < CARDS.length; i++) {
      if (CARDS[i].no === no) { show("quiz"); jump(i); return; }
    }
  }

  // 개념 카드에서 그 개념이 나오는 단문으로
  function toDoc(no) {
    for (var i = 0; i < DOCS.length; i++) {
      if (DOCS[i].no === no) { show("read"); doc(i); return; }
    }
  }
  ...
  $("c-prev").addEventListener("click", function () { cMove(-1); });
  $("c-next").addEventListener("click", function () { cMove(1); });
  $("c-kind").addEventListener("change", cBuild);
  $("c-order").addEventListener("change", cBuild);
  ...
  function doc(i) {
    ...
    $("read-body").innerHTML =
      '<div class="doc-nav">' +
        ...
        '<span class="size-ctl">' +
          '<button type="button" class="wide" data-go="' + (i - 1) + '"' +
            (i === 0 ? " disabled" : "") + ">← 이전</button>" +
          '<button type="button" class="wide" data-go="' + (i + 1) + '"' +
            (i === DOCS.length - 1 ? " disabled" : "") + ">다음 →</button>" +
      ...
      '<div class="doc-head">' +
        '<div class="meta"><span class="n">' + esc(d.no) + "</span>" + stars(d.stars) +
          '<span class="refs"><button type="button" class="ref" data-card="' + esc(d.no) +
          '">두문자 카드 →</button></span></div>' +
        "<h2>" + esc(d.title) + "</h2>" +
  ...
  $("read-body").addEventListener("click", function (e) {
    ...
    if (t.dataset.card !== undefined) return toCard(t.dataset.card);
  ...
  document.addEventListener("keydown", function (e) {
    ...
    if (e.key === "/") { e.preventDefault(); return openSheet(views.con.hidden ? "card" : "con"); }

    if (!views.con.hidden) {
      if (e.key === " " || e.key === "Enter" || e.key === "ArrowDown") {
        e.preventDefault(); cStep();
      } else if (e.key === "ArrowRight") { cMove(1); }
      else if (e.key === "ArrowLeft") { cMove(-1); }
      return;
    }
